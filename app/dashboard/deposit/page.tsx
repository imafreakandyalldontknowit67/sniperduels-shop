...
const COUNTRIES = [
  ...
  { code: 'PW', label: 'Palau' },
  { code: 'PS', label: 'Palestinian Authority' },
  ...
  { code: 'SH', label: 'St Helena, Ascension, Tristan da Cunha' },
...
export default function DepositPage() {
  ...
  // â”€â”€ Card/CashApp deposit (Pandabase) â”€â”€
  async function handleCardDeposit() {
    ...
    try {
      ...
      // method='pending' because the Pandabase sheet hasn't asked the user to pick yet â€”
      ...
      if (win.Pandabase && data.sessionId) {
        const checkout = win.Pandabase.checkout({
          ...
          // it here as a hint, but the webhook is authoritative â€” server fires
          ...
          onPaymentSuccess: (detail: { type?: string; brand?: string; country?: string } = {}) => {
            // Optional client-side hint event â€” the webhook fires deposit_completed
  ...
  // â”€â”€ Crypto deposit (NearPayments) â”€â”€
    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (verifyData.status === 'completed') {
          if (pollRef.current) clearInterval(pollRef.current)
          // Client-side hint â€” the webhook fires deposit_completed server-side
  ...
  async function handleVerify(depositId: string) {
    ...
    try {
      ...
      if (data.status === 'completed') {
        // Client hint â€” server webhook fires the canonical deposit_completed
  ...
  return (
    <div className="flex flex-col items-center min-h-[100dvh] md:min-h-0 px-3 md:px-0 pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="w-full max-w-xl">
        ...
        {/* Crypto Tab Content â€” Currency selector + deposit */}
        {tab === 'crypto' && !cryptoPayment && (
          <>
            <div className="bg-dark-800/50 rounded-xl p-6 mb-6">
